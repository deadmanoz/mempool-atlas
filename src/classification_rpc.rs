...
impl ClassificationRpcClient {
    ...
    pub(crate) fn send_batch(
        ...
        let response = minreq::post(&self.url)
            ...
            .with_body(body)
            .send_lazy()
            .map_err(ClassificationRpcError::Transport)?;
        if response.status_code != 200 {
            return Err(ClassificationRpcError::UnexpectedHttpStatus(
...
fn read_bounded_response(
    ...
    if let Some(expected) = announced_length
        && body.len() != expected
    {
        return Err(ClassificationRpcError::IncompleteResponseBody {
            expected,
            actual: body.len(),
        });
    }
