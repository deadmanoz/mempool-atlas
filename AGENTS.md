...
- `crates/atlas-classifiers/` defines the required MVP classifier contract, transaction shape derivation with the documented dominant-script-type rule, and the registered rule packs, starting with the ordered baseline behavior heuristics. Each pack owns one taxonomy; classifiers receive a parsed `bitcoin::Transaction` and the server parses raw bytes once. It is not a dynamic plugin system.
...
- Current preproduction schema generations intentionally reject stale databases instead of carrying cross-generation logic. The central schema is generation 5 (adds `transaction_classification`, one row per txid and taxonomy). Replace stale state only through the backup-first `reinitialize` targets and reinitialize central state plus every source outbox together. Once production begins, SQLite migrations become append-only.
