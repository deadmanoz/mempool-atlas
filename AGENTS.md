...
- Clean preproduction schemas are central generation 7 and agent generation 4. Stale databases are replaced only through backup-first reinitialization. Production migrations become append-only once real data must be retained.
...
- Agent `max_page_count` bounds the main database only. A frozen checkpoint can coexist with current membership, and transient WAL peak needs separate headroom.
...
- SQLite temp storage stays in memory, but database, WAL, and SHM paths must remain writable.
