...
export const createSnapshotDistributionsView = ({
  ...
  const render = (
    ...
    renderSpectrumChart(
      packageChart,
      model.packageSpectrum,
      FEE_RATE_TICKS,
      metricFormat,
      EMPTY_SNAPSHOT_MESSAGE,
    );
    packageNote.textContent = `Effective ancestor fee rate by ${lensName} bucket: delta-adjusted package fees over package virtual size, the score a miner evaluates.`;
    ...
    dataNote.textContent = `Stacked ${metricNoun} per log payload-size bin by Data protocols bucket · ${countFormat.format(model.totals.carrier.count)} transactions carry OP_RETURN data.`;
