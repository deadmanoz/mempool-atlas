...
describe("packageFeeRate", () => {
  it("divides delta-adjusted package fees by package vsize", () => {
    expect(packageFeeRate(transaction())).toBe(3);
  });

  it("returns zero for a zero package vsize", () => {
    expect(packageFeeRate(transaction({ ancestor_vsize: 0 }))).toBe(0);
  });
});
...
describe("transactionFactPairs", () => {
  ...
  it("omits the OP_RETURN row when the payload is empty", () => {
    ...
    expect(pairs.some(([label]) => label === "OP_RETURN payload")).toBe(false);
  });
});
...
describe("transactionFactSummary", () => {
  it("joins the same facts into one compact line", () => {
    ...
    expect(summary).toContain("package 2 tx · 500 vB @ 3 sat/vB");
