//! Wire-preserving, bounded JSON-RPC batches for policy enrichment.
//!
//! Bitcoin Core's membership RPC remains owned by `rpc`. This module exists
//! because policy enrichment needs the exact distinction between a present
...
pub(crate) struct PolicyRpcClient {
    url: String,
...
impl PolicyRpcClient {
    ...
    pub(crate) fn send_batch(
        ...
        method: &str,
        parameters: &[Value],
    ) -> Result<PolicyRpcBatch, PolicyRpcError> {
        ...
        let requests = parameters
            ...
            .collect::<Vec<_>>();
        let body = serde_json::to_vec(&requests).map_err(PolicyRpcError::EncodeRequest)?;
        let response = minreq::post(&self.url)
            ...
            .with_body(body)
            .send_lazy()
            .map_err(PolicyRpcError::Transport)?;
        if response.status_code != 200 {
            return Err(PolicyRpcError::UnexpectedHttpStatus(response.status_code));
        }
        let (status_code, body) = read_bounded_response(response, self.maximum_response_bytes)?;
    ...
    fn reserve_ids(&self, count: usize) -> Result<Vec<u64>, PolicyRpcError> {
        if count == 0 {
            return Err(PolicyRpcError::EmptyBatch);
        }
        let count = u64::try_from(count).map_err(|_| PolicyRpcError::RequestIdExhausted)?;
        let start = self
            .next_id
            .fetch_update(Ordering::Relaxed, Ordering::Relaxed, |current| {
                current.checked_add(count)
            })
            .map_err(|_| PolicyRpcError::RequestIdExhausted)?;
        Ok((start..start + count).collect())
    }
}

impl fmt::Debug for PolicyRpcClient {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("PolicyRpcClient")
...
#[derive(Debug)]
pub(crate) struct PolicyRpcBatch {
    pub(crate) responses: Vec<Option<PolicyRpcResponse>>,
    pub(crate) response_bytes: usize,
}

#[derive(Debug, Deserialize)]
pub(crate) struct PolicyRpcResponse {
...
impl PolicyRpcResponse {
    pub(crate) fn outcome(&self) -> PolicyRpcOutcome<'_> {
        if self.jsonrpc.as_deref() != Some(JSON_RPC_VERSION) {
            return PolicyRpcOutcome::ProtocolFailure;
        }
        match &self.error {
            ErrorMember::Value(error) => serde_json::from_str::<RpcErrorObject>(error.get())
                .map(|error| PolicyRpcOutcome::RpcError(error.code))
                .unwrap_or(PolicyRpcOutcome::Malformed),
            ErrorMember::Null => PolicyRpcOutcome::Malformed,
            ErrorMember::Missing => match &self.result {
                ResultMember::Value(result) => PolicyRpcOutcome::Value(result),
                ResultMember::Null => PolicyRpcOutcome::Null,
                ResultMember::Missing => PolicyRpcOutcome::Malformed,
            },
        }
    }
}
...
#[derive(Debug)]
pub(crate) enum PolicyRpcOutcome<'a> {
    Value(&'a RawValue),
    Null,
    RpcError(i32),
    ProtocolFailure,
...
fn read_bounded_response(
    ...
    maximum: usize,
) -> Result<(i32, Vec<u8>), PolicyRpcError> {
    let status_code = response.status_code;
    if response.headers.contains_key("transfer-encoding") {
        return Err(PolicyRpcError::UnsupportedTransferEncoding);
    }
    ...
    if let Some(actual) = announced_length
        && actual > maximum
    {
        return Err(PolicyRpcError::ResponseTooLarge { actual, maximum });
    }
    let mut body = Vec::with_capacity(announced_length.unwrap_or(0).min(maximum));
    for byte in &mut response {
        let (byte, _) = byte.map_err(PolicyRpcError::Transport)?;
        if body.len() == maximum {
            return Err(PolicyRpcError::ResponseTooLarge {
    ...
    if let Some(expected) = announced_length
        && body.len() != expected
    {
        return Err(PolicyRpcError::IncompleteResponseBody {
            expected,
            actual: body.len(),
        });
    }
...
fn decode_batch_response(
    ...
    status_code: i32,
    expected_ids: &[u64],
) -> Result<Vec<Option<PolicyRpcResponse>>, PolicyRpcError> {
    let responses = serde_json::from_slice::<Vec<PolicyRpcResponse>>(body).map_err(|source| {
        PolicyRpcError::DecodeResponse {
            status_code,
            source,
        }
    })?;
    if responses.len() > expected_ids.len() {
        return Err(PolicyRpcError::WrongBatchResponseSize);
    }
    let mut by_id = HashMap::with_capacity(responses.len());
    for response in responses {
        let id = response.id;
        if by_id.insert(id, response).is_some() {
            return Err(PolicyRpcError::DuplicateResponseId(id));
        }
    }
    ...
    if let Some(id) = by_id.keys().next().copied() {
        return Err(PolicyRpcError::UnexpectedResponseId(id));
    }
    Ok(ordered)
}

#[derive(Debug, Error)]
pub(crate) enum PolicyRpcError {
    #[error("policy RPC batches cannot be empty")]
    EmptyBatch,
    #[error("policy RPC request IDs are exhausted")]
    RequestIdExhausted,
    #[error("failed to encode policy RPC request: {0}")]
    EncodeRequest(#[source] serde_json::Error),
    #[error("policy RPC HTTP transport failed: {0}")]
    Transport(#[source] minreq::Error),
    #[error("policy RPC returned unexpected HTTP status {0}")]
    UnexpectedHttpStatus(i32),
    #[error("policy RPC response used unsupported Transfer-Encoding")]
    UnsupportedTransferEncoding,
    #[error("policy RPC response ended after {actual} of {expected} declared bytes")]
    IncompleteResponseBody { expected: usize, actual: usize },
    #[error("policy RPC HTTP {status_code} response was not a valid JSON-RPC batch")]
    DecodeResponse {
        status_code: i32,
        #[source]
        source: serde_json::Error,
    },
    #[error("policy RPC response used {actual} bytes, exceeding the {maximum}-byte limit")]
    ResponseTooLarge { actual: usize, maximum: usize },
    #[error("policy RPC batch returned more responses than requests")]
    WrongBatchResponseSize,
    #[error("policy RPC batch returned duplicate response ID {0}")]
    DuplicateResponseId(u64),
    #[error("policy RPC batch returned unexpected response ID {0}")]
...
mod tests {
    ...
    fn result_member_preserves_value_null_and_omission() {
        ...
        assert!(matches!(
            responses[0].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::Null)
        ));
        assert!(matches!(
            responses[1].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::Malformed)
        ));
        assert_eq!(
            match responses[2].as_ref().expect("value result").outcome() {
                PolicyRpcOutcome::Value(result) => result.get(),
                outcome => panic!("unexpected outcome: {outcome:?}"),
            },
            r#"{"ok":true}"#
        );
        assert!(matches!(
            responses[3].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::Malformed)
        ));
    }
    ...
    fn response_outcomes_preserve_error_precedence_and_protocol_failures() {
        ...
        assert!(matches!(
            responses[0].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::RpcError(-5))
        ));
        assert!(matches!(
            responses[1].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::RpcError(-32601))
        ));
        assert!(matches!(
            responses[2].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::ProtocolFailure)
        ));
        assert!(matches!(
            responses[3].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::ProtocolFailure)
        ...
        assert!(matches!(
            responses[4].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::RpcError(-5))
        ));
        assert!(matches!(
            responses[5].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::Malformed)
        ));
    }
    ...
    fn duplicate_unexpected_and_excess_response_ids_are_rejected() {
        ...
        assert!(matches!(
            ...
            Err(PolicyRpcError::WrongBatchResponseSize)
        ));
    }
    ...
    fn empty_batch_is_rejected_before_dispatch() {
        let client = PolicyRpcClient::new(
            "http://127.0.0.1:1/",
            "atlas".to_owned(),
            "secret".to_owned(),
        ...
        assert!(matches!(
            ...
            Err(PolicyRpcError::EmptyBatch)
        ));
    }

    #[test]
    fn concurrent_id_reservations_are_disjoint() {
        let client = Arc::new(PolicyRpcClient::new(
            "http://127.0.0.1:1/",
            "atlas".to_owned(),
            "secret".to_owned(),
            Duration::from_secs(1),
    ...
    fn http_client_rejects_announced_body_over_limit_before_decode() {
        ...
        assert!(matches!(
            error,
            PolicyRpcError::ResponseTooLarge {
                actual,
                maximum: 8
    ...
    #[test]
    fn http_client_accepts_bounded_close_delimited_body() {
        let response = br#"[{"jsonrpc":"2.0","result":null,"id":1}]"#;
        ...
        assert_eq!(batch.response_bytes, response.len());
        assert!(matches!(
            batch.responses[0].as_ref().map(PolicyRpcResponse::outcome),
            Some(PolicyRpcOutcome::Null)
        ));
    }

    #[test]
    fn http_client_rejects_close_delimited_body_at_limit_plus_one() {
        let response = b"123456789";
        let (address, _request, server) =
            serve_once_without_content_length(response, Duration::ZERO);
        let client = PolicyRpcClient::new(
            &format!("http://{address}/"),
        ...
        let error = client
            .send_batch("gettxout", &[json!(["txid", 0, false])])
            .expect_err("oversized close-delimited response");
        server.join().expect("fixture server");
    ...
    fn http_client_rejects_premature_eof_before_declared_length() {
        ...
        assert!(matches!(
            error,
            PolicyRpcError::IncompleteResponseBody {
                expected: declared,
                actual
    ...
    fn http_client_rejects_chunked_framing_even_when_complete() {
        ...
        assert!(matches!(error, PolicyRpcError::UnsupportedTransferEncoding));
    }
    ...
    fn http_client_does_not_follow_redirects() {
        ...
        assert!(matches!(error, PolicyRpcError::UnexpectedHttpStatus(302)));
        assert!(matches!(
    ...
    fn http_client_rejects_non_success_status_even_with_valid_json() {
        ...
        assert!(matches!(error, PolicyRpcError::UnexpectedHttpStatus(500)));
    ...
    fn http_client_applies_its_timeout() {
        ...
        assert!(matches!(error, PolicyRpcError::Transport(_)));
    }
