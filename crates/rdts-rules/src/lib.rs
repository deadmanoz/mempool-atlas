//! `rdts-rules`: a pure evaluator for the seven BIP-110 (Reduced Data Temporary
...
//! clock. [`evaluate_consensus`] applies the deployment window and per-input
//! UTXO grandfathering. [`evaluate_mempool_policy`] mirrors the deployed
//! Knots standard-mempool policy by applying all seven rules without activation
//! gating or grandfathering.
//!
...
//!
//! This crate does not access a chain or UTXO set and does not independently
...
pub use verdict::{
    EvaluationMode, Missing, PrimaryViolation, RuleId, RuleOutcome, RuleVerdict, ScriptKind,
    TxEvidence, Violation,
};
