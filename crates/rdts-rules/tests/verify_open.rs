...
use rdts_rules::{PrevoutFacts, PrevoutSet, RuleId, RuleVerdict, ScriptKind, Violation};
...
fn eval_tx(spk: ScriptBuf, tx: &Transaction) -> rdts_rules::TxEvidence {
    let prevouts = PrevoutSet::from_vec(vec![Some(PrevoutFacts::new(spk, None))]);
    rdts_rules::evaluate_mempool_policy(tx, &prevouts)
}
...
fn p2sh_rule2_evidence_preserves_client_stage_order() {
    ...
    let ev = rdts_rules::evaluate_mempool_policy(&tx, &prevouts);
